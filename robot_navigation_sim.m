...
%
% -------------------------------------------------------------------------
% HONESTY / SCOPE NOTES (please read before assuming this is a drop-in
% replacement for a purchased product):
%   1. This was written from your pasted product description and a still
%      frame of the preview video, not from the seller's actual source
%      code (I don't have access to that). It reproduces the ALGORITHMS
%      named (Theta*, APF, Pure Pursuit, LIDAR, occupancy mapping) with a
%      reasonable, working implementation -- it will not be pixel-identical
%      to the original product's visuals or code structure.
%   2. "SLAM" in this script is occupancy-grid MAPPING only. The robot's
%      pose used for mapping and control comes directly from the ground-
%      truth simulation state, not from a pose estimator. Real SLAM would
%      additionally estimate the robot's own position from noisy sensor
%      data (e.g. via an EKF or particle filter) -- that part is not
%      implemented. Most low-cost "SLAM" demo projects at this price point
%      do the same simplification, but you should know that's what's here.
%   3. The "4-wheel mobile robot" is a simplified kinematic unicycle model
%      drawn with a 4-wheel schematic overlay for visualization. It is not
%      a full multi-body / slip dynamics model.
%   4. I do not have MATLAB or Octave available in the environment I wrote
%      this in, so I could not execute it to confirm it runs error-free.
%      I've written and reviewed it carefully and it follows standard,
%      well-tested formulations of each algorithm, but please run it
%      yourself and treat this as a solid, well-commented starting point
%      you may need to debug/tune rather than a guaranteed-perfect drop-in.
%   5. Requires base MATLAB only (no extra toolboxes): uses inpolygon,
%      conv2, and VideoWriter, all of which ship with core MATLAB.
% -------------------------------------------------------------------------
